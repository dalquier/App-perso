...
function freezeProviderResult(result: ProviderResult): Readonly<ProviderResult> {
  const snapshot = structuredClone(result);
  if (snapshot.usageObserved) Object.freeze(snapshot.usageObserved);
...
export class FakeExecutionProvider implements ExecutionProvider {
  ...
  async execute(request: Readonly<ProviderRequest>, signal: AbortSignal): Promise<Readonly<ProviderResult>> {
    ...
    if (outcome) return freezeProviderResult(outcome.result);
