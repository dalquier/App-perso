...
const replaceMission = (run: PersistedRun, mission: PersistedMission, now: string): PersistedRun => {
  ...
  const missions = Object.fromEntries(Object.entries(changed).map(([id, item]) => {
    ...
    const dependenciesCompleted = dependencies.every((dependency) => changed[dependency]?.state === "completed");
    if (item.state === "blocked_by_dependency" && dependenciesCompleted) return [id, { ...item, state: "ready" as const, attempts: item.attempts.map((attempt) => attempt.attempt === item.current_attempt ? { ...attempt, state: "ready" as const } : attempt) }];
    if (!dependenciesCompleted && (item.state === "ready" || item.state === "prepared_manual")) return [id, { ...item, state: "blocked_by_dependency" as const, attempts: item.attempts.map((attempt) => attempt.attempt === item.current_attempt ? { ...attempt, state: "blocked_by_dependency" as const } : attempt) }];
    return [id, item];
  }));
...
export class ManualExecutionService {
  ...
  markLaunched(run: PersistedRun, jobId: string, now: string): PersistedRun {
    ...
    if (mission.state !== "ready" && mission.state !== "prepared_manual") throw new ManualExecutionError("The job cannot be launched from its current state.", "invalid_state");
    const prepared = mission.state === "ready" ? transitionJob(mission.state, "prepared_manual") : mission.state;
    const launched = transitionJob(prepared, "launched_manual");
    ...
    return replaceMission(run, { ...mission, state: launched, attempts }, now);
  }
  ...
  async importResponse(run: PersistedRun, jobId: string, responseRaw: string, now: string, manuallyConfirmed = false): Promise<PersistedRun> {
    const job = plannedJob(run, jobId); const mission = run.missions[jobId];
    if (job.execution_channel !== "chatgpt_plus_manual") throw new ManualExecutionError("The job is not configured for manual execution.", "wrong_channel");
    if (mission.state === "blocked_by_dependency" || job.depends_on.some((id) => run.missions[id]?.state !== "completed")) throw new ManualExecutionError("All dependencies must be completed before importing a response.", "dependency_blocked");
    if (mission.state !== "launched_manual" && mission.state !== "waiting_response_import") throw new ManualExecutionError("A response can only be imported after an explicit manual launch.", "invalid_state");
    ...
    const completed = transitionJob(mission.state, "completed");
    const attempts = mission.attempts.map((item) => item.attempt === mission.current_attempt ? { ...item, state: completed, result } : item);
  ...
  retry(run: PersistedRun, jobId: string, now: string): PersistedRun {
    const job = plannedJob(run, jobId); const mission = run.missions[jobId];
    if (job.execution_channel !== "chatgpt_plus_manual") throw new ManualExecutionError("The job is not configured for manual execution.", "wrong_channel");
    const retryable = ["completed", "failed_retryable", "failed_terminal", "cancelled"] as const;
