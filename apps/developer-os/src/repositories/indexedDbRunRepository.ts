...
export const DEVELOPER_OS_DB_VERSION = DB_VERSION;
export { CONVERSATION_RUNS_STORE, PROJECTS_STORE } from "../data/indexedDbSchema";
...
const hydrate = (run: PersistedRun): PersistedRun => {
  const copy = structuredClone(run);
  deepFreeze(copy.plan);
  return copy;
};
...
export class IndexedDbRunRepository implements RunRepository {
  ...
  async list(): Promise<PersistedRun[]> {
    ...
    return ((await db.getAll(CONVERSATION_RUNS_STORE)) as PersistedRun[])
  ...
  async get(runId: string): Promise<PersistedRun | undefined> {
    ...
    const run = (await db.get(
      CONVERSATION_RUNS_STORE,
      runId,
    )) as PersistedRun | undefined;
    return run ? hydrate(run) : undefined;
  }

  async create(run: PersistedRun): Promise<void> {
    await this.insertWithoutOverwrite(run);
  }

  async importRun(run: PersistedRun): Promise<void> {
    await this.insertWithoutOverwrite(run);
  }

  private async insertWithoutOverwrite(run: PersistedRun): Promise<void> {
    const db = await this.db;
    const tx = db.transaction(CONVERSATION_RUNS_STORE, "readwrite");
    if (await tx.store.get(run.run_id)) {
      await tx.done;
      throw new RunRepositoryError(
    ...
    await tx.store.add(structuredClone(run));
    await tx.done;
  }

  async update(
    runId: string,
    update: (current: PersistedRun) => PersistedRun,
    ...
    if (!current) {
      await tx.done;
      throw new RunRepositoryError(`Run ${runId} was not found.`, "not_found");
    }
    const next = update(structuredClone(current));
    if (next.run_id !== runId) {
      await tx.done;
      throw new RunRepositoryError(
        "A run identifier cannot be changed.",
        "invalid_data",
      );
    }
