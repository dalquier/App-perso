...
test("mobile resume, secure reference and project import preserve Codex data", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("link", { name: "Nouvelle création rapide" }).click();
  ...
  await page.goto("/settings");
  const download = page.waitForEvent("download"); await page.getByRole("button", { name: "Exporter" }).click(); const exportDownload = await download; expect(exportDownload.suggestedFilename()).toContain("developeros-export");
  const exportedPath = await exportDownload.path(); expect(exportedPath).toBeTruthy();
  const backup = page.waitForEvent("download"); await page.locator("input[type=file]").setInputFiles(exportedPath!); await backup;
  page.once("dialog", (dialog) => dialog.accept()); await page.getByRole("button", { name: "Confirmer" }).click(); await expect(page.getByRole("status")).toContainText("Import terminé");
  expect(await page.evaluate(async () => { const request = indexedDB.open("developeros", 2); const db = await new Promise<IDBDatabase>((resolve) => { request.onsuccess = () => resolve(request.result); }); const value = await new Promise((resolve) => { const get = db.transaction("codexConversations").objectStore("codexConversations").get("e2e-codex"); get.onsuccess = () => resolve(get.result); }); db.close(); return Boolean(value); })).toBe(true);
});
