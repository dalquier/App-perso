...
class ConfigStoreTests(unittest.TestCase):
    def test_add_toggle_rename_remove_and_destination(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIsNone(store.set_drive_destination("drive-1"))
            self.assertEqual(store.load()["driveDestinationBookmark"], "drive-1")
            self.assertEqual(store.set_drive_destination("drive-2"), "drive-1")
