...
L'écran principal contient l'état, la phase courante, les sources, un accès direct `＋` à l'ajout de dossier et l'action de sauvegarde. Pendant une exécution, cette action devient **Arrêter la sauvegarde** : l'opération courante s'achève proprement et le prochain lancement reprend les éléments déjà validés, sans doublon. La destination locale, la liste complète des sources, les exclusions, le test Drive et les archives se trouvent dans **Paramètres**. Ces écrans sont intégrés à la vue principale afin de rester fiables sur Pyto ; `Terminé`, `Retour`, `Annuler` et `Enregistrer` ne dépendent plus d'une feuille modale iOS. Les exclusions sont des lignes indépendantes : ajout court, suppression par balayage et normalisation automatique des extensions.
