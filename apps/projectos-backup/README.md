...
Le bouton unique `Mettre à jour la sauvegarde` enchaîne le miroir local et sa synchronisation Google Drive. L’interface distingue toujours `Miroir local sécurisé`, `Drive en cours` et `Sauvegarde complète et vérifiée`. Elle affiche la phase, le fichier ou lot courant, une progression réelle et conserve une synthèse finale : ajouts, modifications, suppressions, éléments inchangés, reprises, durées et total vérifié. Un bouton `Fermer` reste disponible.
