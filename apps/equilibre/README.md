...
Importer `dalquier/App-perso`, sélectionner le SHA ou la branche candidate exacte, puis utiliser le bouton **Run**. La configuration versionnée `.replit` appelle depuis la racine :

```bash
./start-equilibre.sh
```

Le script exécute un `npm ci` déterministe dans `apps/equilibre`, puis démarre Vite sur `0.0.0.0:${PORT:-5000}` avec `strictPort`. Aucun ancien workspace, script local, Secret Replit, `.env` ou agent IA Replit n'est requis.
