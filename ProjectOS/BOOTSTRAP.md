...
- La question canonique de parallélisation est une réponse décisionnelle dédiée et n’est pas un message intermédiaire de progression. Elle ne doit jamais être fusionnée avec une estimation de temps.
...
6. Charger `ProjectOS/standards/TOOLCHAIN_POLICY.md` pour toute demande liée à un projet logiciel.
7. Charger `ProjectOS/standards/ARTIFACT_DELIVERY_AND_RECOVERY.md` pour toute tâche susceptible de produire une modification, une branche, une Pull Request ou un artefact.
8. Charger `ProjectOS/standards/CODEX_NATIVE_PUBLISHING.md` pour toute tâche exécutée dans Codex Cloud ou publiée par l’interface native Codex.
9. Charger `ProjectOS/standards/CONVERSATION_MEMORY.md` et `ProjectOS/standards/CONVERSATION_ARCHIVE_PIPELINE.md` pour toute nouvelle conversation ProjectOS.
10. Identifier le projet, l’objectif réel et le résultat attendu.
11. Résoudre le projet dans `PROJECT_REGISTRY.md`.
12. Charger son `PROJECT_MANIFEST.md`, s’il existe.
13. Charger les ADR applicables et uniquement la documentation nécessaire.
14. Charger les standards transverses pertinents : qualité, outils, code, documentation et tests.
15. Avant de remettre un prompt opérationnel à Damien, appliquer le bloc de recommandation défini dans `standards/CREDIT_OPTIMIZATION.md`.
16. Avant toute première modification, exécuter le Delivery Preflight : dépôt, référence de base, branche cible, SHA source, canal de livraison, compatibilité des fichiers, plan de récupération et preuve externe attendue.
17. Vérifier l’état vivant des dépôts, branches, Pull Requests, fichiers et exécutions concernés.
18. Consulter Google Drive uniquement pour les ressources explicitement référencées ou nécessaires.
19. Dans la première réponse, présenter uniquement un état rapide des vérifications effectuées : source et branche, références obligatoires chargées, projet identifié ou niveau transverse, anomalies éventuelles et disponibilité pour poursuivre. Ne pas détailler le processus de chargement.
20. Appliquer le régime de consentement défini dans `standards/CONVERSATION_MEMORY.md` :
...
21. Avec Codex, poursuivre sans demander de confirmation supplémentaire. Avec un autre outil, attendre la réponse `oui` ou `non` avant de créer tout artefact permanent de mémoire conversationnelle.
22. Dès que la mémoire est activée, par consentement permanent Codex ou par réponse positive, initialiser l’archive Drive, puis charger sélectivement l’index, la chronologie et les synthèses pertinentes du projet.
23. Si la réponse est `non` dans un régime à consentement ponctuel, poursuivre sans mémoire conversationnelle et sans bloquer le traitement.
24. Après résolution du régime de mémoire, reprendre la demande initiale et appliquer `standards/PARALLEL_EXECUTION.md`. Si plusieurs flux visibles satisfont tous ses critères, envoyer uniquement la question canonique `Cette demande comporte des actions indépendantes. Les paralléliser ?` et attendre la réponse avant de lancer ces flux.
25. Si aucune autorisation de parallélisation n’est nécessaire, ou après résolution de la question, exécuter la méthode ProjectOS jusqu’à la livraison ou au meilleur résultat vérifiable possible.
