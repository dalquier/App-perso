...
- Toute application utilisant Replit applique `standards/REPLIT_RUNTIME_CONTRACT.md` : le mode runtime, le SHA exécuté, le lancement, le port et la Preview native doivent être vérifiables.
...
- Tout prompt ProjectOS produit automatiquement par un script, analyseur, orchestrateur ou script MAÎTRE et contenant `POS =` ou `RPOS =` inclut la directive `MEMORY = OFF` définie dans `standards/CONVERSATION_MEMORY.md` et commence la mission sans question d’enregistrement.
...
- La mémoire conversationnelle est régie par `standards/CONVERSATION_MEMORY.md` et l’archive intégrale par `standards/CONVERSATION_ARCHIVE_PIPELINE.md`. Un consentement permanent spécifique à Codex est actif depuis le 5 août 2026 : toute conversation ProjectOS exécutée avec Codex est enregistrée automatiquement, avec index et synthèse dans GitHub, puis transcription visible et fichiers accessibles directement dans Google Drive, jusqu’à révocation explicite. Une directive explicite `MEMORY = OFF` désactive toutefois l’enregistrement pour cette conversation précise sans révoquer ce consentement permanent.
...
9. Charger `ProjectOS/standards/REPLIT_RUNTIME_CONTRACT.md` pour toute demande concernant Replit, Preview/Webview, Artifact, Workflow de lancement, port, serveur runtime, import GitHub dans Replit, PWA exécutée dans Replit ou création d’une nouvelle application destinée à utiliser Replit.
10. Charger `ProjectOS/standards/ARTIFACT_DELIVERY_AND_RECOVERY.md` pour toute tâche susceptible de produire une modification, une branche, une Pull Request ou un artefact.
11. Charger `ProjectOS/standards/CODEX_NATIVE_PUBLISHING.md` et `ProjectOS/standards/CODEX_GITHUB_RELIABILITY.md` pour toute tâche exécutée dans Codex Cloud, publiée depuis Codex ou destinée à relire, valider ou fusionner un résultat Codex dans GitHub.
12. Charger `ProjectOS/standards/CONVERSATION_MEMORY.md` et `ProjectOS/standards/CONVERSATION_ARCHIVE_PIPELINE.md` pour toute nouvelle conversation ProjectOS.
13. Identifier le projet, l’objectif réel et le résultat attendu.
14. Résoudre le projet dans `PROJECT_REGISTRY.md`.
15. Charger son `PROJECT_MANIFEST.md`, s’il existe.
16. Charger les ADR applicables et uniquement la documentation nécessaire.
17. Charger les standards transverses pertinents : qualité, outils, code, documentation et tests.
18. Avant de remettre un prompt opérationnel à Damien, appliquer le bloc de recommandation défini dans `standards/CREDIT_OPTIMIZATION.md`.
19. Avant toute première modification, exécuter le Delivery Preflight : dépôt, référence de base, branche cible, SHA source, canal de livraison, compatibilité des fichiers, plan de récupération et preuve externe attendue. Pour un flux Codex ↔ GitHub, compléter ce précontrôle par le Reliability Preflight : SHA vivant de base, SHA de tête de PR éventuel, contexte de lancement, ressources mutables réservées et preuves SHA/CI/fusion attendues.
20. Lorsqu’une application utilise Replit, exécuter le `REPLIT RUNTIME PREFLIGHT` défini dans `standards/REPLIT_RUNTIME_CONTRACT.md` avant la première recette et après toute évolution susceptible d’invalider le runtime.
21. Lorsque la tâche dépend d’une capacité d’environnement pour exécuter un test, produire un screenshot, utiliser un navigateur/runtime ou joindre un service externe, exécuter l’Execution Capability Preflight avant de promettre cette preuve et définir un environnement alternatif lorsque nécessaire.
22. Vérifier l’état vivant des dépôts, branches, Pull Requests, fichiers et exécutions concernés. Pour un flux Codex ↔ GitHub, appliquer le Freshness Gate avant la première modification puis à nouveau juste avant publication.
23. Consulter Google Drive uniquement pour les ressources explicitement référencées ou nécessaires.
24. Dans la première réponse, présenter uniquement un état rapide des vérifications effectuées : source et branche, références obligatoires chargées, projet identifié ou niveau transverse, anomalies éventuelles et disponibilité pour poursuivre. Ne pas détailler le processus de chargement.
25. Appliquer le régime de consentement défini dans `standards/CONVERSATION_MEMORY.md` :
    - si la conversation porte la directive explicite `MEMORY = OFF`, désactiver mémoire et archive pour cette conversation, ne poser aucune question d’enregistrement et poursuivre immédiatement la mission après l’amorçage ;
    - sinon, avec Codex, activer automatiquement la mémoire et l’archive intégrale au titre du consentement permanent du 5 août 2026, attribuer un identifiant de session, initialiser le dossier Drive selon le pipeline et terminer la première réponse par `Mémoire Codex : enregistrement activé.` ;
    - sinon, avec tout autre outil, terminer la première réponse par la question exacte `Enregistrer la conversation ?`, sans aucun texte après.
26. Avec `MEMORY = OFF`, ne créer aucun artefact permanent de mémoire conversationnelle et ne pas attendre de confirmation. Avec Codex sans cette directive, poursuivre sans demander de confirmation supplémentaire. Avec un autre outil sans cette directive, attendre la réponse `oui` ou `non` avant de créer tout artefact permanent de mémoire conversationnelle.
27. Dès que la mémoire est activée, par consentement permanent Codex ou par réponse positive, initialiser l’archive Drive, puis charger sélectivement l’index, la chronologie et les synthèses pertinentes du projet.
28. Si la mémoire est désactivée par `MEMORY = OFF` ou par une réponse `non`, poursuivre sans mémoire conversationnelle et sans bloquer le traitement.
29. Après résolution du régime de mémoire, reprendre la demande initiale et appliquer `standards/PARALLEL_EXECUTION.md`. Si plusieurs flux visibles satisfont tous ses critères, envoyer uniquement la question canonique `Cette demande comporte des actions indépendantes. Les paralléliser ?` et attendre la réponse avant de lancer ces flux.
30. Si aucune autorisation de parallélisation n’est nécessaire, ou après résolution de la question, exécuter la méthode ProjectOS jusqu’à la livraison ou au meilleur résultat vérifiable possible en appliquant `standards/PROGRESS_COMMUNICATION.md` et `standards/INCIDENT_LEARNING.md`. Tout incident matériel détecté est dédupliqué puis enregistré dans #87, ou marqué `INCIDENT_CAPTURE_PENDING` dans le handoff lorsque GitHub n’est pas inscriptible depuis l’environnement producteur.
...
- `standards/REPLIT_RUNTIME_CONTRACT.md` pour toute tâche Replit/runtime/Preview ou création d’une application destinée à Replit ;
...
Avec `MEMORY = OFF`, ne charger aucune mémoire conversationnelle ni archive de session uniquement au titre de la continuité conversationnelle ; les références canoniques GitHub restent chargées normalement.
...
Pendant le chargement, chaque message intermédiaire suit `standards/PROGRESS_COMMUNICATION.md` et indique brièvement ce qui a été chargé ou vérifié, l’action en cours, ce qu’il reste à faire et le temps restant estimé. La première réponse reste courte et ne contient qu’un état des vérifications, puis le régime de mémoire applicable.

Avec `MEMORY = OFF`, l’amorçage ne s’arrête pas sur une question de consentement : l’agent poursuit directement la mission. Sans cette directive, l’amorçage conserve l’activation automatique Codex ou la question de consentement pour les autres outils.
...
- Runtime Contract et Replit Runtime Preflight résolus lorsque la tâche l’exige ;
...
- régime de mémoire appliqué : `MEMORY = OFF`, activation automatique Codex ou consentement ponctuel demandé.
